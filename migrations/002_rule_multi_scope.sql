...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes WHERE indexname = 'notification_rules_scope_unique_idx'
  ) THEN
    CREATE UNIQUE INDEX notification_rules_scope_unique_idx
      ON notification_rules (recipient_id, platform_codes, primary_tag_codes);
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes WHERE indexname = 'notification_rules_enabled_idx'
  ) THEN
